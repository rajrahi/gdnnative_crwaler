...
def get_uuid(port: str) -> str: #########
    folder_path = "uuid"
    os.makedirs(folder_path, exist_ok=True)  # Create the folder if it doesn't exist

    file_path = os.path.join(folder_path, f"uuid_{port}.json")
...
class Metrics(object):
    def __init__(self, idx):
        ...
        self.monitoring_counter = Counter(
                                            'scroll_plugin_counter',
                                            'POST API hits from plugin',
                                            [
                                                "account_id", "account_name", "ad_type", "country", "message",
                                                "mode", "network", "plugin_id", "plugin_version",
                                                "project_name", "server_name", "service"
                                            ], registry=None)
...
def send_json(payload, gdn_logger, native_logger, proxy, metric , mode):
    if payload["network"] != "gdn":
        ...
        try:
            ...
            mode = "prod"
            if mode=="dev":
                native_api ="https://native-dev.poweradspy.com/api/adsData"
            else:
            ...
            metric.monitoring_counter.labels(account_id="-",
                                          account_name="-",
                                          ad_type=payload["type"],
                                          country=payload["country"],#hit geolocation.poweradspy.com
                                          message=message,
                                          mode=mode,
                                          network="native",
                                          plugin_id=plugin_id,
                                          ...
                                          service="general_scrapping_py").inc()
        ...
        try:
            ...
            mode = "prod"
            if mode=="dev":
                gdn_api ="https://gdnapi.poweradspy.com/api/adsData"
            else:
...
def update_json(driver, r_session, payload, gdn_logger, native_logger, proxy, metric, uniq_ad_dict,  base_url, input_url, mode):
    ...
    if payload['network'] == 'gdn':
        ...
        try:
            if payload['ad_id'] not in uniq_ad_dict:
                ...
                send_json(payload, gdn_logger, native_logger, proxy, metric , mode)
...
def main(driver, r_session, input_url, base_url, ip_dict, gdn_logger, native_logger, offset, proxy, metric, uniq_ad_dict , mode):
    ...
    for n in list_of_json_gdn + list_of_json_native:
        update_json(driver, r_session, n, gdn_logger, native_logger, proxy, metric, uniq_ad_dict, base_url, input_url ,mode)
...
def scraper_call(proxy_port):
    try:
        ...
        plugin_id = get_uuid(port=port)
        hostname=socket.gethostname()
        print(hostname)
       
        start_http_server(int(port))
        # print(start_http_server(int(port)))
       

        # print(metric.monitoring_counter.labels())
        # mode
        metric.monitoring_counter.labels(account_id="-",
                                        account_name="-",
                                        ad_type='-',
                                        ...
                                        project_name="PAS",
                                        server_name=hostname,
                                        service="general_scrapping_py"
                                    ).inc()

        gdn_logger = setup_logger('gdn_logger', "log/gdn_ios_" + str(proxy) + ".log")
        gdn_ads_logger = setup_logger('gdn_ads_logger', "log/ads_gdn_ios_" + str(proxy) + ".log")
        
        native_logger = setup_logger('native_logger', "log/native_ios_" + str(proxy) + ".log")
        native_ads_logger = setup_logger('native_ads_logger', "log/ads_native_ios_" + str(proxy) + ".log")
        ...
        for i in range(3):
            try:
                driver, r_session, ip_dict = init.init_dc(proxy, metric, gdn_logger, native_logger, ios_agent)
                break
            except:
                pass
        ...
        url_counter = 0
        uniq_ad_dict = dict()
        url_list = open('input/native_urls.txt').read().splitlines()
        ...
        while True:
            for r1_url in url_list:
                try:
                    ...
                    offset = 100000
                    try:
                        ad_count_gdn, ad_count_native = main(driver, r_session, r1_url, base_url, ip_dict, gdn_logger, native_logger, offset, proxy, metric, uniq_ad_dict , mode)
                        ...
                        metric.processed_gdn_url.labels(country=proxy, status="pass", source="ios").inc()
                        metric.processed_native_url.labels(country=proxy, status="pass", source="ios").inc()
                    except Exception as e:
                        print("[GDN_Native Scraper failed] Proxy:", proxy, " Message:", e)
                        metric.processed_gdn_url.labels(country=proxy, status="fail", source="ios").inc()
                        metric.processed_native_url.labels(country=proxy, status="fail", source="ios").inc()
...
def multi_chrome():
    ...
    install_mp_handler()
    pool_list = [("ie", "8000"), ("il", "8001"), ("in", "8002"), ("it", "8003"), ("jp", "8004"), ("kz", "8005"),("ma", "8006"), ("mx", "8007")]
    p = Pool(8)
