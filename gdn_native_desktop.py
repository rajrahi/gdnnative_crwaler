...
def random_click(driver, r_session, r1_url, base_url, ip_dict, gdn_logger, native_logger, offset, proxy, metric, uniq_ad_dict):
# Random click Code
    ...
    if random_link:                    
        ...
        ad_count_gdn, ad_count_native = main(driver, r_session, random_link, base_url, ip_dict, gdn_logger, native_logger, offset, proxy, metric, uniq_ad_dict)
...
def scraper_call(proxy_port):
    try:
        ...
        gdn_logger = setup_logger('gdn_logger', "log/gdn_desktop_" + str(proxy) + ".log")
        gdn_ads_logger = setup_logger('gdn_ads_logger', "log/ads_gdn_desktop_" + str(proxy) + ".log")
        
        native_logger = setup_logger('native_logger', "log/native_desktop_" + str(proxy) + ".log")
        native_ads_logger = setup_logger('native_ads_logger', "log/ads_native_desktop_" + str(proxy) + ".log")
        ...
        for i in range(3):
            try:
                driver, r_session, ip_dict = init.init_dc(proxy, metric, gdn_logger, native_logger, desktop_agent)
                break
            except:
                pass
            ...
            for r1_url in url_list:
                try:
                    ...
                    try:
                        ...
                        for i in range(2):
                            random_click(driver, r_session, r1_url, base_url, ip_dict, gdn_logger, native_logger, offset, proxy, metric, uniq_ad_dict)
                        ...
                        metric.processed_gdn_url.labels(country=proxy, status="pass", source="desktop").inc()
                        metric.processed_native_url.labels(country=proxy, status="pass", source="desktop").inc()
                    except Exception as e:
                        print("[GDN_Native Scraper failed] Proxy:", proxy, " Message:", e)
                        metric.processed_gdn_url.labels(country=proxy, status="fail", source="desktop").inc()
                        metric.processed_native_url.labels(country=proxy, status="fail", source="desktop").inc()
